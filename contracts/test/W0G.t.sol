...
contract W0GTest is Test {
    ...
    function testW0G_Metadata() public view {
    ...
    function testW0G_SetMinter() public {
        vm.prank(owner);
    ...
    function testW0G_SetMinter_RevertIfNotOwner() public {
        vm.prank(user);
        vm.expectRevert(); // Ownable Unauthorized
        w0g.setMinter(user, true);
    }

    function testW0G_MintByOwner() public {
        vm.prank(owner);
        w0g.mint(user, 500 * 10**18);
        assertEq(w0g.balanceOf(user), 500 * 10**18);
    }

    function testW0G_MintByMinter() public {
        vm.prank(owner);
        w0g.setMinter(minter, true);
    ...
    function testW0G_Mint_RevertIfNotOwnerOrMinter() public {
        vm.prank(user);
